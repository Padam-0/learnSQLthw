...
ALTER TABLE person ADD COLUMN dead TYPE INTEGER;

# Add a phone_number to person

ALTER TABLE person ADD COLUMN phone_number TYPE INTEGER;

# Add salary to person (FLOAT)

ALTER TABLE person ADD COLUMN salary TYPE FLOAT;

# Add dob column to person (DATETIME)

ALTER TABLE person ADD COLUMN dob TYPE DATETIME;

# Add purchased_on column to person_pet (DATETIME)

ALTER TABLE person_pet ADD COLUMN purchased_on TYPE DATETIME;

# Add a parent column to pet (INTEGER) that holds id of pet's parent

ALTER TABLE pet ADD COLUMN parent TYPE INTEGER;
...
UPDATE pet SET parent = NULL WHERE id = 3;
...
INSERT INTO person (id, first_name, last_name, age, dead, phone_number, salary, dob) VALUES (3,"Steve", "Jobs",66,1,1234567890,9999,1950-01-01),
	...
	(6,"John","Smith",100,1,1234567890, 0, 1916-01-01);
INSERT INTO pet (id, name, breed, age, dead, dob, parent) VALUES (4,"Roxy","Collie",24,1,1992-01-01,0),
	...
	(8,"James","Cat",30,1,1986-01-01,NULL);
INTSERT INTO person_pet (person_id, pet_id, purchased_on) VALUES (3,4,2005-01-01),(4,5,2006-01-01),(5,6,2007-01-01),(6,7,2008-01-01),(1,8,1990-01-01);
...
SELECT pet.name, person.first_name 
	...
	WHERE id.person = person_id.person_pet AND
	id.pet = pet_id.person_pet AND
	purchased_on.person_pet >= 2004-01-01;

# Write a query that can find the pets that are the children of a given pet

SELECT id, name, parent FROM pet WHERE parent != NULL;
